...
```{r}
#MLP
model <- mlp(STG+SCG+STR+LPR+PEG , UNS, size=c(5,1), maxit=500)
mlp <- predict( model, test_data )
table = table(mlp, test_data$UNS)
summary(model)
model
...
#Here is the performance metrics
data.frame(precision, recall, f1)
table(mlp, test_data$UNS)

test_data$mlp = mlp

p <- plot_ly(test_data, x = ~PEG, y = ~LPR, z = ~SCG, color = ~, colors = c("purple", "orange", "yellow", "green")) %>%
add_markers() %>%
layout(scene = list(xaxis = list(title = 'PEG'), yaxis = list(title = 'LPR'), zaxis = list(title = 'SCG')))
p
```
