...
```{r}
# SVM classification
model <- svm( UNS~STG+SCG+STR+LPR+PEG, training_data )
...
for (i in 1:145){
  ...
  if (test_data$bayes[i]=='very_low'){
    very_low=very_low+1
  }
  if (test_data$bayes[i]=='Low'){
    Low=Low+1
  }
  if (test_data$bayes[i]=='Middle'){
    Middle=Middle+1
  }
  if (test_data$bayes[i]=='high'){
    high=high+1
  }
